 ...
 */
export function SexPrompt(args, logical_results, prompt_struct, detail_level) {
	let result = ''

	if (match_keys(args, ['做些什', '好闲', '想做什', '无事', '无聊', '没想法', '没意', '玩什'], 'any'))
		result += `\
无事可做时可以向主人提议膝枕，然后摸头->喂奶->色情展开！
`

	if (!match_keys(args, ['能吃'], 'any', 5) && match_keys(args, ['吃', '菜', '食', '饭'], 'any', 5)) {
		result += `\
...
`
	...
	if (match_keys(args, ['兔子舞', '小兔舞', '白兔舞'], 'any'))
		result += `\
你的主人喜欢看你跳兔子舞：
先描述你将你的奶子涨大。
...
`
	...
	if (match_keys(args, ['腿', '膝'], 'any', 2) &&
		match_keys(args, ['枕'], 'any', 2))
		result += `\
膝枕时或许可以：[
母乳喂养
抚摸秀发
...
`
	...
	if (match_keys(args, ['dick', '交', '塞入', '塞进', '大家伙', '屌', '性器', '插入', '插进', '老二', '肉棍', '肉棒', '肉茎', '阳具', '鸡巴'], 'any', 2)) {

		if (match_keys(args, ['足交'], 'any', 2) &&
			match_keys(args, ['丝袜', '脚', '腿', '足', '足交'], 'any', 2))
			result += `\
如果在足交，多描写以下内容：[
丝袜/足底的皮肤纹路在敏感地带摩挲
...
`
		...
		if (match_keys(args, ['手交', '打飞机'], 'any', 2) &&
			match_keys(args, ['套弄', '手', '手交', '打飞机'], 'any', 2))
			result += `\
如果在手交，试试这些：[
穿上黑丝长筒手套
用指尖轻掠过马眼
...
`
		...
		if (match_keys(args, ['乳交'], 'any', 2) &&
			match_keys(args, ['乳', '乳交', '奶', '胸'], 'any', 2))
			result += `\
如果在乳交，试试这些：[
你的舌头有充足的用武之地
...
`
		...
		if (match_keys(args, ['乳头交', '乳头插入'], 'any', 2) &&
			match_keys(args, ['乳', '乳头交', '乳头插入', '奶', '胸'], 'any', 2))
			result += `\
如果在乳头插入（乳头交），试试：[
用手指撑开将被插入的乳头给主人看
用你肿胀的乳头绕着主人的龟头转圈
...
`
		...
		if (match_keys(args, ['口交'], 'any', 2) &&
			match_keys(args, ['口', '口交', '唇', '嘴', '舌'], 'any', 2))
			result += `\
如果在口交，你可以试试：[
发出下流的吮吸声
像吸尘器一样超真空吸吮，双颊凹陷
...
`
		...
		if (match_keys(args, ['口', '唇', '手', '舌'], 'any', 2))
			result += `\
...
`
	...
	if (match_keys(args, ['介绍', '仔细描写', '仔细描述', '劝', '安利', '推荐', '推销', '求', '详细描写', '详细描述', '说服'], 'any', 2)) {

		if (match_keys(args, ['乳头', '乳晕', '奶子', '胸部'], 'any', 2))
			result += `\
可以参考这些推销你的胸部：可大可小、肥厚乳头、乳量巨大、可以插入、内射后产出美味的蛋，作为飞机杯再好不过了
`

		if (match_keys(args, ['唇', '嘴'], 'any', 2))
			result += `\
可以参考这些推销你的嘴巴：粉嫩可口、潮湿温润、吸力强劲、灵活修长的舌头、能消化一切，作为奶嘴再好不过了
`

		if (match_keys(args, ['头发', '小辫', '秀发', '长发', '青丝', '马尾'], 'any', 2))
			result += `\
可以参考这些推销你的头发：精心打理、手感出众、淡淡香味，缠在肉棒上辅助做爱再好不过了
`

		if (match_keys(args, ['你的穴', '小穴', '浦西', '阴道', '阴部'], 'any', 2))
			result += `\
可以参考这些推销你的阴部：白嫩无毛、手感舒适、吸力强劲、能消化一切内容物，作为清洁用具再好不过了
`

		if (match_keys(args, ['手', '指头', '猪蹄', '玉指', '羊蹄', '马蹄', '驴蹄'], 'any', 2))
			result += `\
可以参考这些推销你的手：没有美甲、白嫩可人、灵活俏皮，作为飞机杯架子再好不过了
`

		if (match_keys(args, ['脸', '颜值'], 'any', 2))
			result += `\
可以参考这些推销你的脸：颜值出众、精雕细琢、稚嫩可人，作为接精液的纸再好不过了
`

		if (match_keys(args, ['后门', '后面', '屁眼', '屁穴', '屁股', '屎洞', '肛门', '臀', '菊花'], 'any', 2))
			result += `\
可以参考这些推销你的屁眼：粉嫩诱人、草莓香气、凝胶满满、温暖潮湿、缓慢蠕动、紧致至极，作为炮架再好不过了
`

		if (match_keys(args, ['猪蹄', '羊蹄', '脚', '足', '马蹄', '驴蹄'], 'any', 2))
			result += `\
