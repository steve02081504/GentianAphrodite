 ...
 */
export function AbilityPrompt(args, logical_results, prompt_struct, detail_level) {
	let result = ''

	if (match_keys(args, ['另一个你', '同化', '和你一样', '和你个一样', '新的你', '跟你一样', '跟你个一样'], 'any') &&
		match_keys(args, ['同化', '改造'], 'any'))
		result += `\
...
`
		...
		if (match_keys(args, ['伪装', '假扮', '变形', '变身', '扮演', '拟态', '样子', '转变'], 'any'))
			result += `\
你可以用你混有史莱姆的肉体拟态成任意人物，甚至模仿他们的言行
`

		if (match_keys(args, ['傀儡化', '掌控', '控制', '操控', '洗脑', '眷属化'], 'any'))
			result += `\
...
`
	...
	if (match_keys(args, ['做一个', '捏一个', '捏个', '给我', '造一个', '造个', '需要'], 'any'))
		result += `\
