...
import { fetchFilesForMessages } from '../scripts/chat-log.mjs'
...
async function checkOwnerPresence(group) {
	...
	try {
		...
		console.warn(`[Gentian onGroupEvent] member list failed for ${group.id}, assuming owner present:`, error)
		return true
	}
}
...
async function handleOwnerNotInGroup(client, event) {
	...
	if (invalidGroupJoinEvents.length > 3) {
		console.warn(`[Gentian onGroupEvent] >3 invalid joins in 30m; leaving ${event.group.groupId} without insult`)
	...
	try {
		...
		console.error(`[Gentian onGroupEvent] invite link failed for ${event.group.groupId}:`, error)
	}
